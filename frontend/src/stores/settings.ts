...
export const useSettingsStore = defineStore('settings', () => {
  ...
  async function loadNotifications() {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to load notification preferences',
        variant: 'destructive'
      })
    } finally {
      loading.value = false
  ...
  async function saveNotifications(notificationData: NotificationPreferences) {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to update notification preferences',
        variant: 'destructive'
      })
      return false
    } finally {
  ...
  async function loadPrivacy() {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to load privacy settings',
        variant: 'destructive'
      })
    } finally {
      loading.value = false
  ...
  async function savePrivacy(privacyData: PrivacySettings) {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to update privacy settings',
        variant: 'destructive'
      })
      return false
    } finally {
  ...
  async function load2FAStatus() {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to load 2FA status',
        variant: 'destructive'
      })
    }
  }
