...
export function useWebSearch() {
  ...
  // Methods
  const performSearch = async (searchRequest: SearchRequest): Promise<SearchResult[]> => {
    ...
    try {
      ...
      searchState.totalResults = response.data.total_results || response.data.results.length;
  ...
  const getContentForUrls = async (urls: string[]): Promise<Record<string, string>> => {
    try {
  ...
  const findSimilar = async (url: string, numResults: number = 10): Promise<SearchResult[]> => {
    try {
      const response = await findSimilarPages(url, numResults);
      return response.data.results || [];
    } catch (error: any) {
      console.error('Failed to find similar pages:', error);
      throw new Error(error.response?.data?.detail?.message || 'Failed to find similar pages');
    }
  };
  ...
  const checkProviderStatus = async (provider?: SearchProvider): Promise<void> => {
    try {
      const response = await getSearchProviderStatus(provider);

      if (provider) {
        // Single provider status
        providerStatuses.value[provider] = response.data as SearchProviderStatus;
      } else {
        // All providers status
        providerStatuses.value = response.data as Record<string, SearchProviderStatus>;
      }
    } catch (error: any) {
  ...
  const searchGeneral = async (query: string, maxResults: number = 10): Promise<SearchResult[]> => {
  ...
  const searchAcademic = async (query: string, maxResults: number = 10): Promise<SearchResult[]> => {
    return performSearch({
      query,
  ...
  const searchCode = async (query: string, maxResults: number = 10): Promise<SearchResult[]> => {
    return performSearch({
      query,
  ...
  const searchNews = async (query: string, maxResults: number = 10): Promise<SearchResult[]> => {
    return performSearch({
      query,
  ...
  const searchResearch = async (query: string, maxResults: number = 10): Promise<SearchResult[]> => {
    return performSearch({
      query,
  ...
  const getHighScoredResults = (minScore: number = 0.8): SearchResult[] => {
    return searchState.results.filter(result =>
  ...
  return {
    ...
    providerStatuses: providerStatuses.value as Readonly<Record<SearchProvider, SearchProviderStatus>>,
