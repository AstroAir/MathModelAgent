 ...
 */
export function findSimilarPages(url: string, numResults: number = 10) {
...
export function getSearchProviderStatus(provider?: SearchProvider) {
  ...
  return request.get<Record<string, SearchProviderStatus> | SearchProviderStatus>(
    "/search/status",
    { params }
  );
}
...
export function testSearchProvider(provider: SearchProvider) {
  return request.post<{ success: boolean; message: string; response_time?: number }>(
    "/search/test",
    { provider }
  );
}
