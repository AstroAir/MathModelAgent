...
import { Folder, RefreshCw, File, FileText, Download, Archive, Files, Search, LayoutGrid, List, ArrowUpDown, Eye } from 'lucide-vue-next'
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { getFileDownloadUrl, getAllFilesDownloadUrl, getFiles, getFileContent } from '@/apis/filesApi';
...
const isPreviewable = (fileName: string) => {
  ...
  const previewableExts = [
    ...
    'png', 'jpg', 'jpeg', 'gif', 'bmp', 'webp', 'svg' // 支持图片
  ]
...
const zoomIn = () => {
  if (imageZoom.value < MAX_ZOOM) {
    ...
    if (imageZoom.value > 100) {
      ...
      // 应用边界限制
      setTimeout(() => {
        imagePosition.value = clampImagePosition(imagePosition.value.x, imagePosition.value.y)
      }, 0)
    }
  }
}
...
const handleThumbnailMouseMove = (e: MouseEvent) => {
  if (!isThumbnailDragging.value || !imageContainerRef.value || !imageRef.value) return
