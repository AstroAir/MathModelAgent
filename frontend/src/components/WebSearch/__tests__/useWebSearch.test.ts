...
import { performWebSearch, getWebContent, findSimilarPages } from '@/apis/searchApi';
...
describe('useWebSearch', () => {
  ...
  describe('performSearch', () => {
    ...
    it('should handle search errors correctly', async () => {
      ...
      await expect(performSearch(searchRequest)).rejects.toThrow('Search failed');
  ...
  describe('getContentForUrl', () => {
    it('should retrieve content for a single URL', async () => {
      ...
      (getWebContent as any).mockResolvedValue(mockResponse);
    ...
    it('should return null when content retrieval fails', async () => {
      (getWebContent as any).mockRejectedValue(new Error('Failed to get content'));
  ...
  describe('getContentForUrls', () => {
    it('should retrieve content for multiple URLs', async () => {
      ...
      const content = await getContentForUrls(['https://example.com', 'https://test.com']);

      expect(getWebContent).toHaveBeenCalledWith(['https://example.com', 'https://test.com']);
  ...
  describe('findSimilar', () => {
    it('should find similar pages', async () => {
      ...
      (findSimilarPages as any).mockResolvedValue(mockResponse);
      ...
      const results: SearchResult[] = [
        {
          ...
          content: 'This is a test content that is longer than 200 characters and should be truncated to show only the first 200 characters followed by ellipsis to indicate that there is more content available.'
        },
        {
          title: 'Test Result 2',
  ...
  describe('state management', () => {
    it('should clear results correctly', () => {
      ...
      // Set some state
      searchState.results = [{ title: 'Test', url: 'https://example.com' }] as SearchResult[];
