...
def extract_archive(archive_path: str, extract_to: str) -> Tuple[bool, str, List[str]]:
    ...
    try:
        ...
        # 处理 TAR 文件（包括 .tar.gz, .tar.bz2 等）
        elif file_ext in {'.tar', '.gz', '.bz2', '.xz'} or archive_path.name.endswith(('.tar.gz', '.tar.bz2', '.tar.xz')):
            ...
            with tarfile.open(archive_path, mode) as tar_ref:
                ...
                def is_safe_path(path, base_path):
                    """检查解压路径是否安全"""
                    return os.path.realpath(os.path.join(base_path, path)).startswith(base_path)
