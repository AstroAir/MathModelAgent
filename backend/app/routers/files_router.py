...
async def open_folder(task_id: str):
    ...
    # 打开工作目录
    folder_url = Path(work_dir).resolve().as_uri()
...
async def get_file_content(task_id: str, filename: str):
    ...
    if not os.path.exists(file_path):
        ...
        try:
            ...
            ic(f"列出目录失败: {work_dir}, 错误: {e}")
        raise HTTPException(
            status_code=404, detail=f"文件不存在: {filename}，路径：{file_path}"
        )
    ...
    try:
        ...
        if file_size > MAX_SIZE:
            raise HTTPException(
                status_code=400,
                detail=f"文件过大 ({file_size / 1024 / 1024:.1f}MB)，超过10MB限制",
            )
        ...
        if content is None:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="无法解码文件内容，可能是二进制文件"
                )
        ...
        ic(
            f"文件预览: {filename}, 大小: {file_size}, 编码: {used_encoding}, 截断: {truncated}"
        )
